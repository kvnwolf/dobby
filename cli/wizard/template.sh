...
# in the dobby kit repo, copied here VERBATIM (this provenance block is the ONLY
# addition, and it sits ABOVE the library region so it is never part of the
# comparison). The copy exists because the CLI ships to CONSUMERS, where no
# plugin directory exists: `dobby wizard verify <script>` hashes a generated
# wizard's library region — `set -euo pipefail` through the line above the
# "STAGES" marker — against THIS file, so it is the copy every generated wizard
# is measured against (copying from either file is equivalent, and must stay so).
# Re-copy on any change to the canonical file; the two must be byte-identical
# from `set -euo pipefail` down, or `wizard verify` starts rejecting honest
# scripts.
