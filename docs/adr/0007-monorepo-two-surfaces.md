...
- The CLI's distribution is the npm package: `bun install -g @kvnwolf/dobby`, refreshed by `/release`'s final step — the machine runs the latest release, not the live tree (`bun link` remains a dev-only mode for hacking on the CLI). Superseded here: the original bun-link-live model, dropped at v0.1.0 when npm publishing landed.
