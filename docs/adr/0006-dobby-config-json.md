...
**Status:** accepted — schema amended by [ADR-0011](./0011-bundled-toolchain-zero-config.md): the `run` key and task lists were removed (task inference); the file now carries `files[]` + optional `setup[]`/`teardown[]`/`checks[]` extras + optional `tracker` ([ADR-0009](./0009-dual-tracker.md)).
