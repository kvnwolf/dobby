...
**Status:** accepted — **supersedes both pillars of [ADR-0008](./0008-zero-framework-portable-cli.md)** (zero-dependency; no long-running commands). ADR-0008's other decisions stand: the `node:*`-portable core, the `run(argv, cwd)` test seam, releases from the main checkout.
