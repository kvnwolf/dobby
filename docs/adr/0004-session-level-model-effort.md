...
**Status:** accepted for skills inheriting the interactive session; worker-policy ownership is partially superseded by [ADR-0023](./0023-fixed-baseline-workflow-recipe.md).
...
- Current refinement: the interactive main thread remains the **Architect**, and the maintainer still chooses its model/effort manually per task. The five worker agents mirror `baseline-v1`, whose policy source is `cli/src/workflow-recipe.ts`; frontmatter remains the direct-call surface, not an independent policy. Claude Code's operator-level `CLAUDE_CODE_SUBAGENT_MODEL` may override those pins externally.
