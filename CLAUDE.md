...
**The kit assumes Conductor as its execution host.** Sessions run inside a Conductor workspace: `/dobby:onboard` writes `.conductor/settings.toml`, Conductor auto-runs the app (`auto_run_after_setup`), and the coordinator + verifier read the run-script terminal to reach the running app. This is an assumption base, not a portable abstraction — no dual/fallback path for other hosts. (ADR recording this decision is written at `/dobby:wrap`.)
