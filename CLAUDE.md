...
- **Everything in English.** Two skill categories coexist: the work-session **stage** skills (`scope` → … → `wrap`), the worker agents, and their supporting skills are **methodology** — project-agnostic, no references to any specific codebase. The kit ALSO carries **convention** skills (`forms`, `data-fetching`) that encode the user's standard application stack (TanStack Start + Drizzle/Neon + Better Auth, the `@/shared` form/data system) and intentionally reference its module barrels — that coupling is deliberate, not a leak to genericize.
