...
**Dashboard-only (NOT config-as-code, verified):** the status-check confidence THRESHOLD (0–5), the comment header text, auto-approve, and the "Fix with your Agent" buttons. These stay a manual per-project checklist — a `/dobby:bootstrap`-style step could surface them.
