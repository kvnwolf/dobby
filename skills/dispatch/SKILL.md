...
- **Change that needs rigor** → ensure the dev server is up ONCE (capture its URL), then author the **trifecta Workflow** from the `dobby:execute` skill's `references/build-workflow.md` (the shared trifecta component) with a single-element `tasks` array. The implement→review→verify loop applies in full.
