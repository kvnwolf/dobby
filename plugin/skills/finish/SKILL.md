...
## Step 3: Return to main — remove the worktree only if you're standing in one
...
The goal is closed: the run is down, the worktree (if any) and its branch are gone, and main is current. `/dobby:finish` is **terminal** — there is no next stage to hand off to.
