...
Once a lot passes both checks, checkpoint it before moving to the next lot's checkpoint. A separate implementor stages the increment: it writes that lot's resolved sub-keys — changed and explicit-retain files only, from that lot's reviewed final bytes — added to this skill's own sub-key (and, for `anti-slop`, its own `metrics` sub-key), to a **staging path, `.dobby/sweeps.json.pending`, never to the canonical `.dobby/sweeps.json`**. It builds that staging file as the full ledger the canonical file would become: the current canonical content plus this lot's resolved sub-keys, with every other skill's existing entries and every earlier lot's already-written sub-keys passed through byte-for-byte untouched. This writer never stages, commits, or touches the canonical file.

A different reviewer then validates the **staging file alone** — never the canonical file — checking the ledger's top-level schema, this skill's own `rulesVersion`, sorted paths and sub-keys, every final-byte hash the increment adds against that lot's actual final bytes on disk, and that no other skill's sub-key and no earlier lot's sub-key changed.
