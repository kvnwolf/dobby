...
If `.dobby/sweeps.json` does not exist on disk yet, its first full sweep and every approved batch end with a brand-new file that Git still does not track, because `.dobby/` is ignored. After the independent reviewer described in "Writers, reviewers, and final bytes" below validates that file's schema and this skill's own sub-keys and hashes, the **coordinator — never a worker —** runs `git add -f -- .dobby/sweeps.json` exactly once, immediately after that validation, to bring the path under tracking despite the ignore rule. This is the coordinator's one narrow, approved exception to never staging or committing; it applies only when the file did not already exist, only once, and only after independent review has passed.

If `.dobby/sweeps.json` already exists — created by an earlier run of this skill, or by the other sweep skill — it is already an ordinary tracked path: this skill's reviewed updates to its own sub-keys need no `-f` and are picked up by whatever normal commit flow runs afterward, the same as any other tracked file the sweep changed.
...
A manifest writer or manifest reviewer failure follows the same rule: no new ledger finality for this skill's sub-keys; the other skill's existing entries are unaffected either way. A missing `.dobby/sweeps.json` file is not an error; it is the first-full-sweep case above. When the file did not already exist on disk, a failed manifest write or review also means the coordinator never runs the one-time `git add -f`: without independent review passing, the file stays untracked for the next run. When the file already existed (created by the other skill or an earlier run), a failed write or review only leaves this skill's own sub-keys unfinalized — the file's tracking status is unaffected.
