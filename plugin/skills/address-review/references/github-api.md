...
> **Superseded for the address-review loop.** The fetch, reply, batched resolve, re-trigger, and watch are mechanized in the CLI: `bunx dobby review fetch --json`, `bunx dobby review apply --plan <f> --json`, and `bunx dobby pr watch --json`. Prefer them — they carry the five external-system incompatibilities hand-written recipes keep getting wrong: the `$endCursor` name `--paginate` requires, `--watch` being illegal with `--json`, `reviewThreads` being GraphQL-only while the summary is REST, the `[bot]` login mismatch, and Greptile's edited-in-place summary requiring its `Last reviewed commit` SHA to match the PR's current HEAD. What follows is WIRE-LEVEL background for understanding or extending the mechanics. `/dobby:triage`, `/dobby:handoff`, and `/dobby:resolve-conflicts` still use it directly for generic `gh` reads.
