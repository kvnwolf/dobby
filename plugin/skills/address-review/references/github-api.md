...
> **Superseded for the address-review loop.** The fetch, the reply, the batched resolve and the re-trigger below are all mechanized in the CLI: `bunx dobby review fetch --json` (PR identity + adapter detection + drained threads + the edited-in-place summary) and `bunx dobby review apply --plan <f> --json` (replies, one batched `resolveReviewThread`, re-trigger, idempotent on re-run); `bunx dobby pr watch --json` replaces the status-check read. Prefer those — they carry the four `gh` incompatibilities (the `$endCursor` name `--paginate` requires, `--watch` being illegal with `--json`, `reviewThreads` being GraphQL-only while the summary is REST, and the `[bot]` login mismatch) that hand-written recipes keep getting wrong. What follows is the WIRE-LEVEL background: the queries and payload shapes those commands issue, for understanding a field, extending the mechanics by hand, or a one-off read no command exposes. `/dobby:triage`, `/dobby:handoff` and `/dobby:resolve-conflicts` still use it directly for the generic `gh` reads (identify a PR, read its diff and comments).
