...
2. From that exact `workroot`, run **`bunx dobby env --json`**. Require the complete fixed `workflowRecipe` and a positive-integer `workflowRecipe.limits.maxConcurrency`; missing/malformed data → STOP before the first Agent. Never select or reconstruct a model, effort, retry, or concurrency value by hand.

For every `fix`, delegate using those resolved values:

- **Small / scoped** (most review fixes) → spawn `dobby:implementor` (Agent tool, `subagent_type: "dobby:implementor"`). Batch several trivial findings in the same non-overlapping area into ONE implementor instruction. Partition independent direct Agent calls into deterministic sequential batches of at most `workflowRecipe.limits.maxConcurrency`: launch one batch in parallel, await all its results, then launch the next. Retries and replacement Agents consume a slot; serialize shared-state mutations.
- **A fix that must be proven** → write one cohesive fix brief as a task JSON file outside the repo (`{"id","title","spec","areas","verifyRecipe"}`, plus applicable decisions/constraints/testFirst/destructive), then run **`bunx dobby build-plan --task <task.json> --json`**. Require clean preconditions and take the returned `tasks`, `hasTestSuite.value`, and `workRoot`; never hand-build the task array or infer test capability. Author the native build-loop Workflow VERBATIM from `../execute/references/build-workflow.md` and pass only `tasks`, Step 3.1's `devUrl`, `hasTestSuite.value`, `workRoot`, and the complete Step 3.2 `workflowRecipe`. The implement → verify loop, two outer attempts, concurrency cap two, mechanical-first gate, and no-terminal-fix invariant apply in full. Greptile/CodeRabbit then re-reviews the pushed HEAD.
...
Any later direct worker fan-out in this review pass—including a worker for `ci-failed`—uses the same sequential batching rule and resolved cap. Never overlap a direct-Agent batch with a native build Workflow; finish one execution group before launching the next so their combined activity cannot escape the recipe cap. Implementors keep the tree green (build/type/lint); they do NOT commit.
...
- [ ] Before any fix Agent, `bunx dobby up --json` succeeded and supplied `devUrl`/`verifyMode`/`workroot`; `bunx dobby env --json` from that workroot supplied the complete recipe and a valid `limits.maxConcurrency` (failure launched zero Agents)
- [ ] Small/direct fixes were delegated in sequential batches no larger than `maxConcurrency`, only non-overlapping work ran together, and shared-state mutations were serialized
- [ ] Rigorous fixes used scratch task JSON → `bunx dobby build-plan --task … --json` → the VERBATIM native build loop with returned `tasks`, `hasTestSuite.value`, `workRoot`, plus resolved `devUrl` and complete `workflowRecipe`; architect edited no code
