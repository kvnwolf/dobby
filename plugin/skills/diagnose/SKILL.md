...
2. `curl` / HTTP script against a running dev server (bring the run up per `../execute/references/bring-up.md`, then read `devUrl` from `bunx dobby env`; never start a second server).
