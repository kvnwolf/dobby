...
- **Change that needs rigor** → run the build loop, in three commands:
  1. Bring the workspace up per **`../execute/references/bring-up.md`** — `bunx dobby up --json`, reports the environment in one payload. No `dobby.config.json` / no local `node_modules/.bin/dobby` → the project was never onboarded: stop and point the user at `/dobby:onboard`. `ok:false` → STOP and report the `reason` (the closed enum: `not-a-git-repo` · `config-unreadable` · `install-failed` · `worktree-copy-failed` · `setup-extra-failed` · `neon-creds-missing` · `neon-branch-failed` · `liveness-timeout`, plus `degradedCommand` when it offers one); don't dispatch against a workspace that never came up. `instructions[]` non-empty → follow the reference (rename then start, in order) and re-run `up --json` before continuing; a repeated `start` on the second call means the process never registered — stop per the reference's stop rule. Take `devUrl` (null for a library / CLI / plugin like dobby with no run script), `verifyMode` (`url` / `programmatic` — when it's `programmatic` the verifier verifies programmatically instead of against a URL), and `workroot`.
...
- [ ] Rigor path brought the workspace up per `../execute/references/bring-up.md` (STOPping on `ok:false`, following non-empty `instructions[]` and re-running before continuing, taking `devUrl` / `verifyMode` / `workroot`), then `bunx dobby build-plan --task <task.json> --json` — no runtime value was hand-derived
