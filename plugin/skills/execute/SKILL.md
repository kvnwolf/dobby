...
- **`browserPane`** — the kit's cmux browser pane ref, null when cmux is absent. It decides the manual-setup auth surface below (`dobby:qa` reads the same field, so they converge).
...
- [ ] Manual-setup gate honored at end of Step 2: `none` skips silently; steps prompt (AskUserQuestion, in-stage) and block every worker until the user confirms setup in the ONE surface `browserPane` selects
